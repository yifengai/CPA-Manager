...
3. 希望直观看到每个账号的周限额剩余额度、周限额重置、是否受限。
...
| 功能         | 说明                                            |
| ------------ | ----------------------------------------------- |
| 余量刷新     | 显示最近一次账号余量刷新完成的北京时间          |
| 用量刷新     | 显示最近一次今日消耗刷新完成的北京时间          |
| 刷新余量     | 查询所有 Codex 账号的最新余量，不修改 auth 文件 |
| 账号池保护   | 先预览 request-log 风险账号，确认后停用风险账号 |
...
筛选区支持组合筛选，账号类型只在有数据时显示：

| 筛选 | 说明                                     |
| ---- | ---------------------------------------- |
| 状态 | 健康、受限、异常、未知                   |
| 开关 | 启用、停用                               |
| 类型 | free、plus、pro、未知类型等账号类型      |
| 排序 | 剩余额度、周限额重置、存活时间、账号名称 |
...
| 标签         | 含义                                                           |
| ------------ | -------------------------------------------------------------- |
| 已重置       | 当前时间已经超过周限额重置时间，或刷新时检测到今天已进入新周期 |
| 今天         | 今天内重置                                                     |
| 明天         | 明天重置                                                       |
| 2天后至7天后 | 对应日期重置                                                   |
| 未知         | 没有可靠周限额重置时间                                         |
...
| 周限额                    | 若存在短周期和周周期，会同时显示 `5小时限额` 与 `周限额`        |
| 当前额度周期消耗          | 按周限额重置时间往前 7 天，从本地 Usage 记录估算成功请求 Tokens |
...
这个字段不是官方账单，而是本地 Usage 数据反推结果。计算方式：

1. 先取账号的周限额重置时间。
2. 从重置时间往前推 7 天，作为当前额度周期窗口。
3. 在本地 Usage 记录中按账号文件、账号邮箱、账号标识匹配调用记录。
4. 只统计窗口内的成功请求 Tokens。
5. 用 `已统计 Tokens / 单账号满额 Tokens` 展示进度条，默认单账号满额为 `4M Tokens`。

可信度含义：

| 标签     | 含义                                                                |
| -------- | ------------------------------------------------------------------- |
| 可信度高 | 本地统计已用比例和页面周限额已用比例接近，可作为主要参考            |
| 可信度中 | 两者存在差异，或缺少周限额已用百分比，只适合作为参考                |
| 可信度低 | 本地统计和周限额差异较大，通常说明历史 Usage 数据不完整或匹配不充分 |
| 缺少依据 | 没有可靠周限额重置时间，暂不可计算周期窗口                          |

如果看到“本地记录与周限额差异较大”，优先理解为本地历史用量没有收全，不代表账号真实没有消耗。

## 8. 刷新与保护规则

当前规则：

1. 【刷新余量】只查询最新状态，不写回 auth 文件，也不会自动停用账号。
2. 周限额余量为 `0%`、`1-3%`、受限或异常时，页面会归类到对应标签，提示建议处理。
3. 需要移出调用池时，可以手动停用、批量停用，或点击【账号池保护】预览后确认。
4. 已停用账号不进入调用池。
5. 已停用账号仍可查询周限额余量和周限额重置。

这样做的目的是把“查询”和“写入停用”分开，避免刷新页面时产生不可见副作用。

### 8.1 账号池保护

【账号池保护】会先读取 CLIProxyAPI 的 request-log 做预览，并只处理能识别到 `provider=codex`、`auth_id=xxx.json` 的调用记录。预览不会修改任何账号；只有点击确认后，才会把建议停用账号写入 `disabled=true`。

触发规则：

1. 最近请求出现 `401`、认证失败或 token 失效：自动停用。
2. 最近请求出现 `403`、无权限或访问被拒绝：自动停用。
3. 最近请求出现 `429`、rate limit、quota exceeded 或 limit reached：自动停用。
4. 最近请求出现超时、`5xx`、上游错误：连续失败达到 2 次才自动停用。
5. 如果账号最近已有成功请求，会中断连续失败计数，避免误伤偶发失败。

这个功能适合在大批量调用前或发现失败率突然升高后手动执行，用来减少异常账号继续进入调用池。
...
```text
这是一个 CLIProxyAPI 的 Docker 管理面板，重点增强了 Codex 账号周限额余量、周限额重置、账号池估算、失败记录清理和请求日志分析。
