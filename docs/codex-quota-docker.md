...
| 账号池保护   | 扫描最近 request-log，自动停用 401、403、429 或连续失败账号 |
...
1. 周限额余量为 `0%` 时，默认自动停用。
2. 周限额余量为 `1-3%` 时，默认自动停用，避免尾量账号继续进入调用池。
3. 账号受限时，默认自动停用。
4. 账号异常时，默认自动停用。
5. 已停用账号不进入调用池。
6. 已停用账号仍可查询周限额余量和周限额重置。

这样做的目的是避免受限或异常账号继续参与调用，减少失败请求和无效重试。

### 8.1 账号池保护

【账号池保护】会读取 CLIProxyAPI 的 request-log，并只处理能识别到 `provider=codex`、`auth_id=xxx.json` 的调用记录。
