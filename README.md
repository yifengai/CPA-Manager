...
本分支基于 CPA-Manager / CPAMC 增强，重点新增了 **Codex 账号余量、重置时间、账号池余量估算、今日消耗、账号启停、失败记录清理、请求日志分析** 等功能。它不包含 CLIProxyAPI 本体，需要你已经部署好 CLIProxyAPI。
...
7. 如果已经开启 CLIProxyAPI request-log，进入【请求日志】查看最近任务和请求详情。
