...
本分支基于 CPA-Manager / CPAMC 增强，重点新增了 **Codex 账号周限额余量、重置时间、账号池余量估算、今日消耗、账号启停、账号池保护、失败记录清理、请求日志分析** 等功能。它不包含 CLIProxyAPI 本体，需要你已经部署好 CLIProxyAPI。
...
- 账号类型筛选：free、plus、pro，有数据才显示
- 余量分布筛选：0%、1-5%、6-20%、21-50%、51-80%、81-90%、91-100%
- 重置时间筛选：已重置、今天、明天、2 天后至 7 天后、未知
- 存活周期筛选：`<1天`、`1-3天`、`3-7天`、`7-14天`、`14天以上`、未知
- 单账号刷新、启用、停用、归档删除
- 批量启用、批量停用、批量删除、刷新已选账号
- 账号池保护：扫描 request-log，自动停用 401、403、429 或连续失败账号
- 清除失败调用记录
...
6. 点击【账号池保护】，让系统根据最近 request-log 自动停用明确不可继续承接的账号。
...
```text
这是一个 CLIProxyAPI 的 Docker 管理面板，重点增强了 Codex 账号余量、重置时间、账号池估算、今日消耗、失败记录清理和请求日志分析。
